...
**Compliance tagging happens automatically.** The alert generated by rule `5763` came pre-tagged with GDPR, HIPAA, PCI DSS, NIST 800-53, and TSC references, with zero manual effort on the analyst's part (Section 4.3). In practice, that kind of cross-referencing is often its own tedious, manual compliance task; here it's just metadata attached at the moment the alert fires.

**`wazuh-logtest` catches mistakes before they go live.** Rule `100100` could be tested against real, previously captured log lines before it ever touched live traffic (Section 4.5), which meant its logic could be confirmed correct with no risk of either flooding the dashboard with false positives or, worse, quietly failing to fire the next time the attack actually happened.
...
**AI assistance**
- Claude (Anthropic): used throughout the project for troubleshooting guidance: https://claude.ai
