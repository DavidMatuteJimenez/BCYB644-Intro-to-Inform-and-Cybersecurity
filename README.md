...
It's built around four main components: an **agent** that sits on each monitored machine and forwards log data, a central **manager** that decodes that data and checks it against rules, an **indexer** that stores the alerts, and a **dashboard** that gives you a web interface to actually look at them.
...
**Wazuh's primary role in incident response is Identification and Detection.** It continuously collects endpoint and security logs, analyzes them against detection rules, correlates related events, and generates alerts that give analysts actionable evidence of potentially malicious activity. This project demonstrates that role directly through an SSH brute-force attack against a simulated victim machine, where Wazuh detected repeated authentication failures, identified the brute-force pattern, and used a custom correlation rule to elevate a subsequent successful login to a high-severity potential account-compromise alert.

Wazuh also provides features that can **support other stages of the incident response lifecycle**, including Configuration Assessment for Preparation, Active Response for Containment, File Integrity Monitoring for detecting post-compromise changes, and continued monitoring during Recovery. However, those capabilities were not directly demonstrated in this lab. The hands-on portion of this project intentionally focuses on Wazuh's core strength: **detecting and correlating suspicious activity and turning raw endpoint events into actionable security alerts.**
...
**Note:** Because of the RAM constraints on my host machine, VM1 ended up playing double duty. It's both the Wazuh manager and the "victim" endpoint being monitored. This works because the Wazuh manager also has a built-in local agent, identified as agent `000`, which can monitor the manager host without requiring a separate agent installation, so I didn't need a separate agent install. What it actually monitors still depends on the manager's configuration and enabled modules, but by default it's enough to pick up local log sources like `/var/log/auth.log`. In an actual production setup, the manager and the endpoints it watches would normally be separate machines, each running their own agent.
...
The goal of this experiment was to evaluate Wazuh's detection capability against an SSH brute-force attack, determine how its default rules handled both failed authentication attempts and a subsequent successful login, and then improve that detection capability by creating a custom correlation rule, since a successful login occurring shortly after a burst of failures is a strong indicator of possible account compromise, even though it doesn't prove compromise on its own.
...
No match, no matter what I tested. Turned out this wasn't a typo, it was a conceptual mistake. `<id>` matches a value pulled from *inside* a log's fields, not the ID of a rule that already fired. Since rule `5763`'s ID isn't part of the log text itself, this could never match. What I actually needed was `<if_sid>` and `<if_matched_sid>`. Fixing the tags got me the working rule:
...
The working rule uses `<if_sid>5715</if_sid>` to make the successful SSH authentication event the current event being evaluated, while `<if_matched_sid>5763</if_matched_sid>` requires a recent brute-force alert to have already fired for that same agent, within Wazuh's correlation window, not necessarily the immediately preceding event. `<same_source_ip />` then ensures both events originated from the same source IP, so it's not just any brute-force alert and any login happening near each other in time. This allows Wazuh to correlate the successful login with the preceding brute-force activity rather than treating the two events independently. When it fires, it jumps to level 12 (way up from `5715`'s level 3) and attaches both MITRE techniques: **T1110** (Brute Force) for the attack method, and **T1078** (Valid Accounts) since the attacker's now operating with real, valid credentials. That second one is arguably the scarier half, since a valid-account login is a lot harder to tell apart from normal activity going forward.
...
It's worth being precise about what the custom rule actually changed here: the primary improvement was not an increase in the number of events Wazuh detected, but an increase in the contextual severity assigned to the successful authentication event. Rule `100100` doesn't give Wazuh visibility into anything it couldn't already see, both `5763` and `5715` fired under the default ruleset before I wrote anything. What it changes is how the system interprets those two events in relation to each other, correlating them into a single high-severity signal instead of leaving them as two separate, unremarkable-looking alerts.
...
| `5763`              |    10 | 8 failed attempts from the same source IP cross the frequency threshold           | T1110, Brute Force                                  |
| `5715`              |     3 | Any successful SSH login (default, no context)                                    | T1078, Valid Accounts; T1021, Remote Services      |
| `100100` (custom)   |    12 | Successful login (`5715`) following recent brute-force activity (`5763`) from the same source IP | T1110, Brute Force; T1078, Valid Accounts |
...
Wazuh's primary strength in this project is **Identification and Detection**. Its ability to collect endpoint telemetry, decode logs, apply detection rules, correlate related events, and generate alerts makes it particularly useful for identifying suspicious activity during an incident.

Wazuh's broader feature set can support other phases of the incident response lifecycle, but those capabilities should be distinguished from what this project actually demonstrated. Configuration Assessment can support **Preparation** by identifying security misconfigurations before an incident occurs. Active Response can support **Containment** by automatically taking action against detected threats, such as blocking an attacking IP. **File Integrity Monitoring can support Investigation and Eradication** by identifying unauthorized changes that may have occurred after an attacker gained access, such as modifications to configuration files, binaries, or other monitored resources, that may then need to be investigated and remediated. Detection itself is not eradication: Wazuh provides the visibility that can assist an analyst during eradication, while the actual removal and remediation would normally be performed by administrators or other security tools.

**Recovery is outside the direct scope of this lab.** In a real incident, Wazuh could support recovery by continuing to monitor the affected endpoint after remediation and confirming that suspicious authentication activity has stopped. For example, analysts could continue monitoring for additional `5763` or `100100` alerts after resetting the compromised account and blocking the attacking source. However, this project did not perform those remediation or recovery activities.

The practical demonstration focused specifically on **detection and analysis**: identifying an SSH brute-force attack, correlating the failed authentication attempts with a subsequent successful login, and improving Wazuh's default detection capability through custom rule `100100`.

The project also demonstrates a **Lessons Learned** concept. After observing that Wazuh's default rules treated the successful login as a low-severity event despite the preceding brute-force activity, I identified a detection gap and developed a custom correlation rule to address it. This demonstrates how findings from one investigation can be used to improve future detection.
...
**Room to build past the defaults.** The default ruleset provides a strong baseline, but custom rules can extend Wazuh's detection capabilities to address environment-specific requirements. Building `100100` showed me the correlation tags (`if_sid`, `if_matched_sid`, `same_source_ip`) are flexible enough to actually patch a real gap: a successful login getting the same severity as any other one, even right after a brute-force attempt.
...
None of these are actually Wazuh's fault, just the tradeoff of running it on Apple Silicon, which it wasn't originally built for. Although Wazuh is free and open-source, deployment on an ARM64 virtualized environment introduced additional compatibility and storage considerations worth checking before committing time to an install.
